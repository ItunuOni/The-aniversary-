...
function conditionalAnalyticsPlugin(): Plugin {
  ...
  return {
    ...
    transformIndexHtml: {
      order: "pre",
      handler(html) {
        if (!env.VITE_ANALYTICS_ENDPOINT) {
          return html.replace(
            /<script[^>]*%VITE_ANALYTICS_ENDPOINT%[^>]*><\/script>/i,
            ""
          );
        }
        return html;
...
const plugins = [react(), tailwindcss(), jsxLocPlugin(), vitePluginManusRuntime(), vitePluginManusDebugCollector(), conditionalAnalyticsPlugin()];
